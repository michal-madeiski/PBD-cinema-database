...
WITH movie_version_screening_count AS (
    ...
    GROUP BY mv.fk_movie_id, mv.fk_version_id
),
...
JOIN movie_screening_max msm ON mvsc.fk_movie_id = msm.fk_movie_id AND mvsc.screening_count = msm.max_screening_count
JOIN movie m ON mvsc.fk_movie_id = m._id
JOIN version v ON mvsc.fk_version_id = v._id
ORDER BY mvsc.screening_count DESC, m.title ASC