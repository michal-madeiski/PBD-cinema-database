...
WITH ticket_cash_payments AS (
    SELECT
        ...
        COUNT(*) FILTER (WHERE p.type = 'cash')::numeric AS cash_count
    FROM ticket t
...
SELECT 
    ROUND((tp.cash_count / tp.total) * 100.00, 2) AS percent_tickets_cash,
    ROUND((pp.cash_count / pp.total) * 100.00, 2) AS percent_products_cash,
    ROUND(((tp.cash_count + pp.cash_count) / (tp.total + pp.total)) * 100.00, 2) AS percent_all_cash
FROM ticket_cash_payments tp, product_cash_payments pp;